...
เมื่อ View Page Source ดู พบว่ามี HTML comment ที่ระบุ credentials `test:test` อยู่ตรงใกล้ๆ login form
...
เมื่อลองดู traffic ใน Burp Suite จะเห็นว่าหน้านี้มีการโหลดไฟล์ `/script.js` และเรียกข้อมูลจาก URL `/api.php?action=get_userinfo`
...
ในไฟล์ `script.js` มีโค้ดอยู่ 3 ส่วนหลักๆ ส่วนแรกเป็นโค้ดที่จะรันเมื่อโหลดหน้าเว็บเสร็จ โดยจะเป็นการ fetch URL `api.php?action=get_userinfo` ซึ่งก็คือ GET request ตามที่เราเห็นใน Burp Suite ด้านบน
...
หลังจากผมเปลี่ยน JWT แล้วลอง refresh หน้าเว็บ พบว่ายังมีสิทธิ์เป็น user "test" อยู่เหมือนเดิม จึงลองลบ cookie ดัวอื่นๆ ออกหมด ให้เหลือแค่ `remember_me` แล้ว refresh หน้าเว็บอีกรอบ
...
เมื่อเรามีสิทธิ์ admin แล้วจะสามารถเข้าถึงหน้า `https://web1.ctf.p7z.pw/admin.php` ได้ ซึ่งหน้าเว็บจะเป็นฟอร์มสำหรับรับค่าจำนวนธนบัตรและสกุลเงินที่จะพิมพ์
